...
async function validateStored(row:typeof modelConfigs.$inferSelect){const result=await validateModelConfig({provider:row.provider,model:row.model,baseUrl:row.baseUrl,apiKey:await decryptApiKey(row.encryptedApiKey)});const checkedAt=new Date();await (await getDb()).update(modelConfigs).set({validationStatus:result.valid?"valid":"invalid",validationError:result.error,lastValidatedAt:checkedAt,updatedAt:checkedAt}).where(eq(modelConfigs.id,row.id));return {...row,validationStatus:result.valid?"valid" as const:"invalid" as const,validationError:result.error,lastValidatedAt:checkedAt,updatedAt:checkedAt}}
...
export async function DELETE(request:Request){try{const id=new URL(request.url).searchParams.get("id");if(!id)return Response.json({error:"缺少 id"},{status:400});await (await getDb()).delete(modelConfigs).where(and(eq(modelConfigs.id,id),eq(modelConfigs.userId,userId(request))));return Response.json({ok:true})}catch(e){return Response.json({error:e instanceof Error?e.message:"删除失败"},{status:500})}}

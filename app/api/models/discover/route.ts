...
export async function POST(request:Request){
  try{
    ...
    if(body.configId){const [stored]=await getDb().select().from(modelConfigs).where(and(eq(modelConfigs.id,String(body.configId)),eq(modelConfigs.userId,userId(request)))).limit(1);if(!stored||!stored.enabled||stored.validationStatus!=="valid")return Response.json({error:"找不到已验证的模型配置"},{status:404});provider=stored.provider;apiKey=await decryptApiKey(stored.encryptedApiKey);baseUrl=(stored.baseUrl||"").replace(/\/$/,"")}

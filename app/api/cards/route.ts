...
export async function POST(request:Request){
  try{
    ...
    for(const input of body.cards){
      ...
      created.push({id:card.id,type:"原题",front:card.front,back:card.back,path:"AI 解析 / 待归类",tags:uniqueNames.map(([,name])=>name),interval:"新卡片",tone:"orange",createdAt:now,lastReviewAt:null,nextReviewAt:null,memoryStatus:"new",sourceUrl:sourceAssetId?`/api/assets/${sourceAssetId}`:null,detail});
    }

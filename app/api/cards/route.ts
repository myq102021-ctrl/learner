...
export async function PATCH(request:Request){
  try{
    ...
    const rawModel=body.thinkingModel&&typeof body.thinkingModel==="object"?body.thinkingModel:null;const thinkingModel=rawModel?{name:String(rawModel.name||"").trim(),description:String(rawModel.description||"").trim(),steps:Array.isArray(rawModel.steps)?rawModel.steps.map((item:unknown)=>String(item).trim()).filter(Boolean):[]}:null;const detail={stem,options,answer,solution,thinkingModel:thinkingModel&&(thinkingModel.name||thinkingModel.description||thinkingModel.steps.length)?thinkingModel:null};const front=[stem,...options.filter(option=>!stem.replace(/\s+/g,"").includes(option.replace(/\s+/g,"")))].join("\n");const back=stored.cardType==="memorization"?answer:`答案：${answer}\n\n解析：${solution}`;const now=new Date();
    await db.update(learningCards).set({front,back,explanation:JSON.stringify(detail),updatedAt:now}).where(and(eq(learningCards.id,id),eq(learningCards.userId,uid)));await db.delete(learningCardTags).where(eq(learningCardTags.learningCardId,id));
    ...
    return Response.json({card:{id,type:stored.cardType==="question"?"原题":stored.cardType==="memorization"?"背诵":"知识点",front,back,path:"AI 解析 / 待归类",tags:uniqueNames.map(([,name])=>name),sourceUrl:stored.sourceAssetId?`/api/assets/${stored.sourceAssetId}`:null,detail,updatedAt:now}});
